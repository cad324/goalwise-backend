...
from accountability_score.serializer import (
    AccountabilityScoreSerializer,
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            if self.is_valid_field(field_name, UserMetrics):
                ...
                logger.info(
                    f"Updated {field_name} for user {instance.user.id}. New value: {getattr(instance, field_name)}"
                )
            ...
            return Response(
                ...
                data={"message": "UserMetrics not found"},
            )
        except Exception as e:
            logger.error(
                f"Unexpected error occurred while updating UserMetrics for user {request.user.id}."
            )
            return Response(
